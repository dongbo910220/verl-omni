...
def _mean(rows: list[dict], field: str) -> float:
    values = [_as_float(row, field) for row in rows]
...
def analyze_eval(args) -> None:
    ...
    metric_names = ("cer", "wer", "score", "duration_s", "valid_audio", "runaway", "no_eos", "trailing_silence")
    ...
    for field in metric_names:
        values = [_as_float(candidate, field) - _as_float(base, field) for _, _, base, candidate in paired_rows]
