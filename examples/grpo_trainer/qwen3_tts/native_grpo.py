...
def cached_interleaved_codec0_logits(talker, batch: InterleavedTalkerBatch) -> torch.Tensor:
    ...
    for action_index in range(1, max_actions):
        ...
        cache_position = torch.tensor([sequence_index], dtype=torch.long, device=batch.inputs_embeds.device)
        output = talker.model(
...
def probability_consistency(
    ...
    passed = bool(np.isfinite(pearson) and diff_mean < CONSISTENCY_DIFF_LIMIT and pearson > CONSISTENCY_PEARSON_LIMIT)
    return {
...
def _prepare_auto_generation_inputs(wrapper, text: str, group_size: int):
    ...
    aligned_prefix = (
        torch.cat((tts_pad.expand(-1, codec_prefix.shape[1] - 2, -1), tts_bos), dim=1) + codec_prefix[:, :-1]
    )
    prefill = torch.cat((projected[:, :3], aligned_prefix, projected[:, 3:4] + codec_prefix[:, -1:]), dim=1)
...
def parse_generation_result(
    result, eos_token_id: int, num_codebooks: int = NUM_CODEBOOKS
) -> tuple[
    list[torch.Tensor],
    list[torch.Tensor],
    list[torch.Tensor],
    ...
    raw_logits = torch.stack([value[:, -1] if value.ndim == 3 else value for value in result.logits], dim=1).float()
    ...
    frames = (
        torch.stack(frame_list, dim=1).long() if frame_list else sampled.new_zeros((sampled.shape[0], 0, num_codebooks))
    )
...
def _sample_native_rollout_batch(
    ...
    *,
    ...
    suppress = [token for token in range(talker.config.vocab_size - 1024, talker.config.vocab_size) if token != eos_id]
    ...
    if capture_prefill:
        ...
        prefill_capture["active_adapters"] = list(active) if isinstance(active, list | tuple) else active
...
def decode_rollout_audio(model, audio_codes: list[torch.Tensor]) -> tuple[list[np.ndarray], int]:
    ...
    decoded, sample_rate = model.speech_tokenizer.decode([{"audio_codes": codes} for _, codes in nonempty])
...
def grpo_loss(
    ...
    *,
    ...
    for actor, reference, advantage in zip(actor_logprobs, reference_logprobs, advantages, strict=True):
