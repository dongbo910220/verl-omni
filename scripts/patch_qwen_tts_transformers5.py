...
_SUPPORTED_QWEN_TTS_VERSION = "0.1.1"
...
def patch_file(path: pathlib.Path) -> None:
    if not path.exists():
        raise FileNotFoundError(f"Expected qwen-tts source file is missing: {path}")
    ...
    if remaining:
        raise RuntimeError(f"Transformers 5 compatibility patch is incomplete for {path}: {remaining}")
...
def main() -> None:
    if len(sys.argv) == 1:
        ...
        installed_version = version("qwen-tts")
        if installed_version != _SUPPORTED_QWEN_TTS_VERSION:
            raise RuntimeError(f"This patch supports qwen-tts {_SUPPORTED_QWEN_TTS_VERSION}, got {installed_version}.")
