...
def test_eval_pairs_the_same_complete_sample_set(tmp_path):
    ...
    assert report["paired_candidate_minus_base"]["cer_capped"]["mean"] == pytest.approx(-0.1)
    assert report["model_card_cer"]["candidate_relative_change"] == pytest.approx(-0.5)
    assert report["model_card_cer"]["paired_outcomes"] == {"better": 2, "equal": 0, "worse": 0}
    assert report["model_card_cer"]["prompt_averaged_across_seeds"] == {
        "prompt_count": 2,
        "seeds_per_prompt": [1],
        "base_mean": pytest.approx(0.2),
        "candidate_mean": pytest.approx(0.1),
        "candidate_minus_base_mean": pytest.approx(-0.1),
        "candidate_minus_base_bootstrap_95_ci": pytest.approx([-0.1, -0.1]),
        "paired_outcomes": {"better": 2, "equal": 0, "worse": 0},
    }
    assert report["checks"]["exact_paired_generation_seeds"] is True


def test_eval_derives_model_card_cer_cap_for_legacy_rows(tmp_path):
    base_path = tmp_path / "base.jsonl"
    candidate_path = tmp_path / "candidate.jsonl"
    output = tmp_path / "eval.json"
    common = {
    ...
    _write_jsonl(base_path, [common | {"cer": 1.4}])
    _write_jsonl(candidate_path, [common | {"cer": 0.8}])
    ...
    report = json.loads(output.read_text())
    assert report["base"]["cer_capped"] == 1.0
    assert report["candidate"]["cer_capped"] == 0.8
    assert report["paired_candidate_minus_base"]["cer_capped"]["mean"] == pytest.approx(-0.2)
