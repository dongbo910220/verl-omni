...
def _load_patch_module():
    ...
    path = root / "scripts/patch_qwen_tts_transformers5.py"
...
def test_patch_fails_when_expected_source_file_is_missing(tmp_path):
    module = _load_patch_module()

    with pytest.raises(FileNotFoundError, match="source file is missing"):
        module.patch_file(tmp_path / "missing.py")
