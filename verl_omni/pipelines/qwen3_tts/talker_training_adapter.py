...
def _speaker_embedding(model, batch_size, device, dtype):
    ...
    if cached is None:
        ...
        if not path:
            raise RuntimeError("Set actor_rollout_ref.model.override_config.tts_spk_embed_path.")
...
def _inject_tts_actor_inputs(output, tokenizer, prompt_length, response_length, model_inputs):
    extra = getattr(output, "extra_fields", None) or {}
    codes, text_ids = extra.get("tts_audio_codes"), extra.get("tts_text_ids")
    if codes is None:
        return model_inputs
    if text_ids is None:
        text = extra.get("tts_text")
        if text is None:
            return model_inputs
        ...
        text_ids = torch.as_tensor(text_ids, dtype=torch.long).reshape(-1)
    ...
    response_size, text_size = min(codes.shape[0], response_length), min(text_ids.numel(), prompt_length)
    audio_buffer = torch.zeros((1, response_length, 16), dtype=torch.long)
    text_buffer = torch.zeros((1, prompt_length), dtype=torch.long)
    audio_buffer[0, :response_size] = codes[:response_size]
    text_buffer[0, :text_size] = text_ids[:text_size]
    model_inputs.update(
        {
            "tts_audio_codes": audio_buffer,
            "tts_text_ids": text_buffer,
            "response_len": torch.tensor([response_size]),
            "text_len": torch.tensor([text_size]),
        }
    )
    return model_inputs


def patch_agent_loop_inputs() -> None:
    """Preserve rollout codec payloads when verl builds actor inputs."""
    from verl.experimental.agent_loop.agent_loop import AgentLoopWorker

    targets = [AgentLoopWorker]
    try:
        from verl.trainer.ppo.v1.agent_loop_tq import AgentLoopWorkerTQ

        targets.append(AgentLoopWorkerTQ.__ray_metadata__.modified_class)
    except (AttributeError, ImportError):
        pass

    for target in targets:
        original = target._compute_multi_modal_inputs
        if getattr(original, "_verl_qwen3_tts_patch", False):
            continue

        def _patched(self, output, input_ids, _original=original):
            model_inputs = _original(self, output, input_ids)
            return _inject_tts_actor_inputs(
                output,
                self.tokenizer,
                int(self.rollout_config.prompt_length),
                int(self.rollout_config.response_length),
                model_inputs,
            )

        _patched._verl_qwen3_tts_patch = True
        target._compute_multi_modal_inputs = _patched
